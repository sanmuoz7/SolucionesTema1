...
```{r}
diferentes_animales_m <- setdiff(rownames(mammals),rownames(Animals2))
diferentes_animales_m #Animales que se encuentran en mammals pero no en Animals2
#Hay 4

diferentes_animales_a <- setdiff(rownames(Animals2),rownames(mammals))
diferentes_animales_a #Animales que se encuentran en Animals2 pero no en mammals
#Hay 7
```
...
```{r}
#La diferencia es que algunos son los mismos pero con alguna errata en el nombre
```
...
#Si, presentan el mismo comportamiento
```
...
#b
any(is.na(cabbages)) #No tiene ningún valor perdido
...
qqPlot(log(cabbages$VitC)) #Si, verifica la distribucion gaussiana
...
plot(cabbages$VitC ~ cabbages$HeadWt) #No tienen relacion lineal
```
...
Modeloc39 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages, subset = which(Cult == "c39")) #Para la calabaza c39

Modeloc52 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages, subset = which(Cult == "c52")) #Para la calabaza c52
